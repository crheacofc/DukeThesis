...
TimeDerivativeS::TimeDerivativeS(const InputParameters & parameters)
...
Real
TimeDerivativeS::computeQpResidual()
{
  double density = _density.density(_u[_qp]);
  return _test[_i][_qp] * _porosity[_qp] * density * _u_dot[_qp];
}

Real
TimeDerivativeS::computeQpJacobian()
{
  double density = _density.density(_u[_qp]);
...
void
TimeDerivativeS::computeJacobian()
{
  if (_lumping)
  {
