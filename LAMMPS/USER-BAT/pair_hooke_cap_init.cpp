...
PairHookeCapInit::PairHookeCapInit(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
...
PairHookeCapInit::~PairHookeCapInit()
{
  delete [] svector;
...
void PairHookeCapInit::compute(int eflag, int vflag)
{
 ...
 //std::cout<<inum<<std::endl;
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (r<radsum*maxSeparationDistanceRatio){ //particles must be within x-number radii of each other
          const double volLi1000 = 1333.333333*M_PI*radi*radi*radi*surfaceLiquidContentInitial[i];
    	    const double volLj1000 =1333.333333*M_PI*radj*radj*radj;//*surfaceLiquidContentInitial[j];
          ...
          const double contactAngleEff = 0.5 * contactAngle;
          ...
          double norm1 = delx*rsqinv;
          double norm2 = dely*rsqinv;
          double norm3 = delz*rsqinv;

          // APPLY CAPILLARY FORCES IN DIRECTION ALONG INCREMENTAL NORMAL
          f[i][0] += Fcapilary*norm1;
          f[i][1] += Fcapilary*norm2;
          f[i][2] += Fcapilary*norm3;

          f[j][0] -= Fcapilary*norm1;
          f[j][1] -= Fcapilary*norm2;
          f[j][2] -= Fcapilary*norm3;
        }
...
void PairHookeCapInit::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairHookeCapInit::settings(int narg, char **arg)
{
  if (narg != 12) error->all(FLERR,"Illegal pair_style comman");
  ...
  if (kn < 0.0 || kt < 0.0 || gamman < 0.0 || gammat < 0.0 ||
    ...
    std::cout<<"Reading in particle information"<<std::endl;
     minSeparationDistanceRatio =  force->numeric(FLERR,arg[6]);
     maxSeparationDistanceRatio =  force->numeric(FLERR,arg[7]);
     surfaceTension =  force->numeric(FLERR,arg[8]);
     fluidViscosity =  force->numeric(FLERR,arg[9]);
     contactAngle =  force->numeric(FLERR,arg[10]);
     SaturationFile = arg[11];
     //READ STUFF FOR SATURATION

     std::ifstream fileIN;
          fileIN.open(SaturationFile);
          std::string lineA;
          int lines=0;
          while(fileIN.good()){
          ...
          fileIN1.open(SaturationFile);

          //
          //read data file
          std::vector<std::vector<double>> ValuesSat(lines,std::vector<double>(3));
          while(fileIN1.good()){

              for(int i=0;i<lines;i++){
                  for (int j=0;j<3;j++){
                      fileIN1 >> ValuesSat[i][j];
                  }
              }

          }
          fileIN1.close();
          //surfaceLiquidContentInitial.clear();
          surfaceLiquidContentInitial.push_back(0.0); //Since atom ID starts at 1
          for(int i = 0; i < lines; i++){
              surfaceLiquidContentInitial.push_back(ValuesSat[i][2]);
          }
          // std::cout<<"Finished reading in particle information"<<std::endl;
          //std::cout<<"We have this number of lines in the sat file: "<<lines<<std::endl;
...
void PairHookeCapInit::coeff(int narg, char **arg)
{
...
void PairHookeCapInit::init_style()
{
  int i;
...
void PairHookeCapInit::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairHookeCapInit::init_one(int i, int j)
{
  if (!allocated) allocate();
...
void PairHookeCapInit::write_restart(FILE *fp)
{
...
void PairHookeCapInit::read_restart(FILE *fp)
{
...
void PairHookeCapInit::write_restart_settings(FILE *fp)
{
...
void PairHookeCapInit::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairHookeCapInit::reset_dt()
{
  dt = update->dt;
}
...
double PairHookeCapInit::single(int i, int j, int itype, int jtype,
                                    double rsq,
...
int PairHookeCapInit::pack_forward_comm(int n, int *list, double *buf,
...
void PairHookeCapInit::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double PairHookeCapInit::memory_usage()
{
...
void *PairHookeCapInit::extract(const char *str, int &dim)
{
