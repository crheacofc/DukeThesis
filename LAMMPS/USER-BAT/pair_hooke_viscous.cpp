...
PairHookeViscous::PairHookeViscous(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
...
PairHookeViscous::~PairHookeViscous()
{
  delete [] svector;
...
void PairHookeViscous::compute(int eflag, int vflag)
{
...
void PairHookeViscous::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairHookeViscous::settings(int narg, char **arg)
{
  if (narg != 6) error->all(FLERR,"Illegal pair_style comman");
  ...
  if (kn < 0.0 || kt < 0.0 || gamman < 0.0 || gammat < 0.0 ||
   ...
   VelFieldFileY = arg[8];
    ...
    std::ifstream fileIN12;
    fileIN12.open(fluidViscosityFile);
    std::string lineA1;
    int lines2=0;
    while(fileIN12.good()){
        while(getline(fileIN12,lineA1)){
            std::istringstream streamA1(lineA1);
            lines2++;
        }
    }

    //string filename;
    std::ifstream fileIN22;
    fileIN22.open(fluidViscosityFile);

    //
    //read data file
    std::vector<std::vector<double>> ValuesfluidViscosity(lines2,std::vector<double>(2));
    while(fileIN22.good()){

        for(int i=0;i<lines2;i++){
            for (int j=0;j<2;j++){
                fileIN22 >> ValuesfluidViscosity[i][j];
            }
        }
    ...
    fileIN22.close();
    for(int i = 0; i < lines2; i++){
        fluidViscosity.push_back(ValuesfluidViscosity[i][1]);
...
void PairHookeViscous::coeff(int narg, char **arg)
{
...
void PairHookeViscous::init_style()
{
  int i;
...
void PairHookeViscous::init_list(int id, NeighList *ptr)
{
  if (id == 0) list = ptr;
...
double PairHookeViscous::init_one(int i, int j)
{
  if (!allocated) allocate();
...
void PairHookeViscous::write_restart(FILE *fp)
{
...
void PairHookeViscous::read_restart(FILE *fp)
{
...
void PairHookeViscous::write_restart_settings(FILE *fp)
{
...
void PairHookeViscous::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairHookeViscous::reset_dt()
{
  dt = update->dt;
}
...
double PairHookeViscous::single(int i, int j, int itype, int jtype,
                                    double rsq,
...
int PairHookeViscous::pack_forward_comm(int n, int *list, double *buf,
...
void PairHookeViscous::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double PairHookeViscous::memory_usage()
{
...
void *PairHookeViscous::extract(const char *str, int &dim)
{
